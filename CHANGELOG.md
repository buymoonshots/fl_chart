...
* **BUGFIX** Fix `fitInsideHorizontally` not working correctly for vertical line labels at chart edges. Labels with `Alignment.topCenter` were overflowing horizontally, and labels with `Alignment.bottomCenter` were not positioned correctly. Now both horizontal and vertical positioning work as expected.
